...
/**
 * Build an SDP offer for a viewer consuming from the SFU (server sendonly).
 * Used when a viewer watches a WHIP stream via the broadcast signaling.
 */
function buildViewerSdpOffer(transport, consumers) {
    const { iceParameters, iceCandidates, dtlsParameters } = transport;
    const fingerprint = dtlsParameters.fingerprints[dtlsParameters.fingerprints.length - 1];
    const mids = [];
    ...
    for (const consumer of consumers) {
        ...
        for (const codec of consumer.rtpParameters.codecs) {
            ...
            if (codec.parameters && Object.keys(codec.parameters).length > 0) {
                mediaSection.fmtp.push({
                    payload: codec.payloadType,
                    config: Object.entries(codec.parameters).map(([k, v]) => `${k}=${v}`).join(';'),
                });
            }
            for (const fb of codec.rtcpFeedback || []) {
                mediaSection.rtcpFb.push({
                    payload: codec.payloadType,
                    type: fb.type,
                    subtype: fb.parameter || undefined,
                });
        ...
        mediaSection.payloads = pts.join(' ');

        for (const ext of consumer.rtpParameters.headerExtensions || []) {
            mediaSection.ext.push({ value: ext.id, uri: ext.uri });
        }
        ...
        sdpObj.media.push(mediaSection);
    }
...
/**
 * Create an SFU consumer transport + consumers for a viewer, return SDP offer.
 * Used by broadcast-server when a viewer watches a WHIP stream.
 *
 * @param {string} roomId - SFU room ID (e.g., 'stream-42')
 * @param {string} viewerPeerId - Unique peer ID for this viewer
 * @returns {{ sdpOffer: string, transportId: string, consumerIds: string[] } | null}
 */
async function createSfuViewerOffer(roomId, viewerPeerId) {
    if (!sdpTransform) return null;

    const room = webrtcSFU.rooms?.get(roomId);
    if (!room || room.producers.size === 0) return null;

    // Create a WebRTC transport for sending media to the viewer
    const transport = await room.router.createWebRtcTransport({
        listenIps: [{ ip: config.mediasoup.listenIp, announcedIp: config.mediasoup.announcedIp }],
        enableUdp: true,
        enableTcp: true,
    ...
    room.transports.set(`${viewerPeerId}-${transport.id}`, transport);

    transport.on('dtlsstatechange', (state) => {
        if (state === 'closed') {
            room.transports.delete(`${viewerPeerId}-${transport.id}`);
        }
    });

    // Create consumers for each producer in the room
    const consumers = [];
    const consumerIds = [];
    ...
    for (const [producerId, { producer }] of room.producers) {
        ...
        try {
            ...
            room.consumers.set(consumer.id, { consumer, peerId: viewerPeerId });
            ...
            console.warn(`[WHIP] Failed to create consumer for ${producerId}:`, err.message);
        }
    }
    ...
    const sdpOffer = buildViewerSdpOffer(transport, consumers);
...
/**
 * Handle a viewer's SDP answer for an SFU consumer transport.
 */
async function handleSfuViewerAnswer(roomId, viewerPeerId, transportId, answerSdpStr) {
    if (!sdpTransform) throw new Error('sdp-transform not available');

    const room = webrtcSFU.rooms?.get(roomId);
    if (!room) throw new Error('Room not found');

    const transportKey = `${viewerPeerId}-${transportId}`;
    const transport = room.transports.get(transportKey);
    if (!transport) throw new Error('Transport not found');

    const answerSdp = sdpTransform.parse(answerSdpStr);
    const dtlsParams = extractDtlsParameters(answerSdp);

    await transport.connect({ dtlsParameters: dtlsParams });
}

/**
 * Clean up a viewer's SFU consumer transport and consumers.
 */
function cleanupSfuViewer(roomId, viewerPeerId, transportId, consumerIds) {
    const room = webrtcSFU.rooms?.get(roomId);
    if (!room) return;
...
module.exports = {
    ...
    handleWhipDelete,
    handleWhipOptions,
    createSfuViewerOffer,
    handleSfuViewerAnswer,
    cleanupSfuViewer,
    hasSfuProducers,
    sessions,
    cleanupSession,
